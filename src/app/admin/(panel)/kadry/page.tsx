...
export default function AdminSquadsPage() {
  ...
  const [editingId, setEditingId] = useState<Id<"players"> | "new" | null>(null);
  ...
  async function handleRemove(id: Id<"players">, name: string) {
    resetFeedback();
    if (!window.confirm(`Usunąć zawodnika ${name}? Zdjęcie też zostanie skasowane.`)) {
      return;
    }
    try {
    ...
    <>
      ...
      <section className="mt-8 rounded-lg border border-border bg-card p-5">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          >
            {busy ? "Dodawanie…" : `Dodaj ${bulkPreview.length || ""} zawodników`}
          </Button>
